...
WeatherRepository weatherRepositoryImpl(Ref ref) {
...
class WeatherRepositoryImpl implements WeatherRepository {
    ...
    return Result.guard(() async {
      ...
      try {
        ...
        if (response.result.header.resultCode != '00') {
          throw AppException.server('초단기 실황 에러: ${response.result.header.resultMsg}');
        }

        final items = response.result.body.items.item;
        if (items.isEmpty) {
          throw const AppException.emptyData('초단기실황 데이터가 없습니다.');
        }
  ...
  @override
  Future<Result<List<HourlyWeather>>> getUltraSrtForecastList({
    required int nx,
    required int ny,
  }) async {
    return Result.guard(() async {
      ...
      try {
        ...
        if (response.response.header.resultCode != '00') {
          throw AppException.server('초단기 예보 에러: ${response.response.header.resultMsg}');
        }

        final items = response.response.body.items.item;
        if (items.isEmpty) {
          throw const AppException.emptyData('초단기예보 데이터가 없습니다.');
        }
  ...
  @override
  Future<Result<DailyShortTermWeather>> getShortTermMinMaxTemps({
    required int nx,
    required int ny,
  ...
  @override
  Future<Result<List<DailyShortTermWeather>>> getShortTermForecast({
    required int nx,
    required int ny,
  }) async {
    return Result.guard(() async {
      ...
      try {
        ...
        if (e is AppException) rethrow;
        throw AppException.server('단기 예보 (일별)를 가져오는 데 실패했습니다: $e');
      }
    });
  }

  @override
  Future<Result<MidTermWeather>> getMidTermWeather({
    ...
    return Result.guard(() async {
      try {
        ...
        if (taResponse.response.header.resultCode != '00' ||
          ...
          throw const AppException.emptyData('중기 예보 데이터가 유효하지 않거나 비어 있습니다.');
        }
        ...
        if (e is AppException) rethrow;
        throw AppException.server('중기 예보를 가져오는 데 실패했습니다: $e');
      }
    });
  }
}
